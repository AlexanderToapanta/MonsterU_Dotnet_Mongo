...
namespace Monster_University.Controllers
{
    public class ControladorCarreraController : Controller
    {
        ...
        public ActionResult Crear(FormCollection form)
        {
            try
            {
                var idFromForm = (form["codigo"] ?? string.Empty).ToString();
                var idToUse = string.IsNullOrWhiteSpace(idFromForm) ? CD_Carrera.Instancia.GenerarNuevoId() : idFromForm;

                var c = new Carrera
                {
                    codigo = idToUse,
                    nombre = (form["nombre"] ?? string.Empty).ToString(),
                    creditosMaximos = int.TryParse(form["creditosMaximos"], out var max) ? max : 0,
                    creditosMinimos = int.TryParse(form["creditosMinimos"], out var min) ? min : 0
                };
        ...
        public ActionResult Editar(FormCollection form)
        {
            try
            {
                var c = new Carrera
                {
                    codigo = (form["codigo"] ?? string.Empty).ToString(),
                    nombre = (form["nombre"] ?? string.Empty).ToString(),
                    creditosMaximos = int.TryParse(form["creditosMaximos"], out var max) ? max : 0,
                    creditosMinimos = int.TryParse(form["creditosMinimos"], out var min) ? min : 0
                };
        ...
        private byte[] GenerarPdfReporte(List<Carrera> carreras, string nombre, int? creditosMin, int? creditosMax)
        {
            using (var ms = new System.IO.MemoryStream())
            {
                ...
                // Encabezados
                var celdaEncabezado1 = new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase("Código", fontEncabezado));
                ...
                {
                    var celdaCodigo = new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase(carrera.codigo ?? "", fontNormal));
                    celdaCodigo.HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER;
                    celdaCodigo.Padding = 5;
                    tabla.AddCell(celdaCodigo);

                    var celdaNombre = new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase(carrera.nombre ?? "", fontNormal));
                    celdaNombre.Padding = 5;
                    tabla.AddCell(celdaNombre);

                    var celdaMin = new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase(carrera.creditosMinimos.ToString(), fontNormal));
                    celdaMin.HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER;
                    celdaMin.Padding = 5;
                    tabla.AddCell(celdaMin);

                    var celdaMax = new iTextSharp.text.pdf.PdfPCell(new iTextSharp.text.Phrase(carrera.creditosMaximos.ToString(), fontNormal));
        ...
        // Acción adicional para obtener una carrera específica (útil para AJAX)
        [HttpGet]
        public JsonResult ObtenerCarreraPorCodigo(string codigo)
        {
            try
            {
                var carrera = CD_Carrera.Instancia.ObtenerCarreraPorId(codigo);
                if (carrera != null)
                {
                    return Json(new
                    {
                        ...
                        carrera = new
                        {
                            ...
                            creditosMinimos = carrera.creditosMinimos,
                            creditosMaximos = carrera.creditosMaximos
                        }
